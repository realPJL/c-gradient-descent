...
float biasDerivative(float predictedValue[], float actualValue[], int numberOfValues) {
    float sum;                          // Sum of all errors * 2
    float error[numberOfValues];        // Array containing all calculated errors
    int i;                              // Index
    float result;                       // Result being returned to caller
    float errorFeature[numberOfValues]; // Array containing all error * 2
